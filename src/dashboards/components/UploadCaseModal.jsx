...
export default function UploadCaseModal({ open, onClose, onSuccess }) {
  ...
  const [paso, setPaso] = useState(1); // 1 = subir, 2 = revisar, 3 = éxito
  ...
  const handleAnalizar = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || "Error al analizar el documento. Intenta de nuevo."
      );
    } finally {
      setCargando(false);
    }
  ...
  const handleConfirmar = async () => {
    if (!titulo.trim() || !descripcion.trim() || criterios.length === 0) {
      setError("Completa el título, descripción y al menos un criterio");
      return;
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || "Error al guardar el caso. Intenta de nuevo."
      );
    } finally {
      setCargando(false);
    }
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      ...
      <DialogContent sx={{ mt: 2 }}>
        ...
        {paso === 1 && (
          <Box sx={{ textAlign: "center", py: 4 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              La IA analizará el documento y extraerá el título, descripción y
              criterios de evaluación automáticamente.
            </Typography>
            ...
            <label htmlFor="upload-case-file">
              <Button
                ...
                sx={{
                  ...
                  color: "darkgreen",
                  "&:hover": { borderColor: "green", backgroundColor: "#f0f7f0" },
                }}
              >
                Seleccionar Archivo
              </Button>
        ...
        {/* ── PASO 2: Revisar y editar datos ── */}
        {paso === 2 && (
          <Box sx={{ py: 2 }}>
            {datos?.rubrica_detectada ? (
              <Chip
                label="Rúbrica detectada en el documento"
                color="success"
              ...
              <Chip
                label="Rúbrica sugerida por IA (no estaba en el documento)"
                color="warning"
                sx={{ mb: 2 }}
              />
            )}
            ...
            <TextField
              ...
              value={descripcionRubrica}
              onChange={(e) => setDescripcionRubrica(e.target.value)}
              fullWidth
              multiline
              rows={2}
              sx={{ mb: 3 }}
        ...
        {paso === 3 && (
          <Box sx={{ textAlign: "center", py: 4 }}>
            ...
            <Typography variant="body1" color="text.secondary">
              El caso "{titulo}" se ha guardado con su rúbrica y {criterios.length}{" "}
              criterios de evaluación.
            </Typography>
          </Box>
        )}
      ...
      <DialogActions sx={{ p: 2 }}>
        ...
        {paso === 2 && !cargando && (
          <>
            <Button onClick={() => setPaso(1)} sx={{ color: "gray" }}>
              Volver
            </Button>
            <Button
              variant="contained"
